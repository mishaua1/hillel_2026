...
filtered_cars = sorted(
    [
        ...
        if specs[1] >= min_year and specs[2] >= min_engine and specs[4] <= max_price
    ],
    key=lambda car: car[5]
)

for car in filtered_cars[:5]:
    name, color, year, engine, car_type, price = car
    print(f"{name}: {color}, {year}, {engine}L, {car_type}, ${price:,}")